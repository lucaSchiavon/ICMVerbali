...
## Addendum 2026-05-14 — B.11 firma Impresa Appaltatrice via magic-link

**Lavori chiusi**: transizione `FirmatoCse → FirmatoImpresa` tramite link condivisibile con token che permette al referente dell'impresa di firmare senza account sul sistema. Aggiunta detail view `/verbali/{id}` read-only per visualizzare un verbale firmato (entrambe le firme renderizzate). Nessuna integrazione SMTP/SMS: il link viene generato lato server al momento della firma CSE e copiato in clipboard dal CSE che lo condivide via WhatsApp/email/SMS come preferisce.

**Niente NuGet aggiunti**: riuso completo dello stack B.10 (`signature_pad` JS, `IFirmaStorageService`, MudBlazor dialog).

**Decisioni di design ratificate**:

1. **Magic-link token (capacità) invece di login impresa**. Il referente impresa NON ha utenza nel sistema. L'accesso al flusso di firma è autorizzato dal solo possesso del token (GUID v7, 128 bit, non enumerabile) presente nell'URL. Scelta motivata da: zero attrito per l'impresa, nessuna gestione utenti/password/claim, coerenza con la prassi reale (l'impresa non sempre è in cantiere quando il CSE finisce). Rischi accettati: chi inoltra il link concede di fatto la capacità di firma (stesso rischio della firma cartacea su foglio condiviso); non c'è verifica forte dell'identità del firmatario (lo è del rappresentante impresa, fuori dal sistema, regolato dal contratto). Le alternative scartate: login autenticato (sovradimensionato per imprese che fanno 10 cantieri/anno), firma in presenza dal device CSE (richiede co-presenza fisica, prassi non sempre rispettata).

2. **Tabella `FirmaToken`** (migration `002_AddFirmaToken.sql`): `Id`, `VerbaleId` FK, `Token` UNIQUE (GUID v7 indipendente da `Id`), `ScadenzaUtc`, `UsatoUtc` nullable, `CreatedAt`. Indice su `VerbaleId`. Stesso pattern di FK NO ACTION del resto del progetto. Il `Token` è distinto da `Id` per non esporre l'ID interno e per poter ruotare il token rigenerandone uno nuovo in futuro senza toccare la PK.

3. **Scadenza 48h configurabile via `appsettings`**: `FirmaTokenOptions.ScadenzaOreDefault = 48`, sezione `FirmaToken` di appsettings, override per ambiente. Razionale: 48h copre il caso "firmo CSE giovedì sera, impresa firma sabato mattina"; per cantieri con timing stretto si può abbassare, per cantieri rilassati si può alzare, senza ricompilare. Una scadenza c'è sempre per evitare token "eterni" inoltrati per errore.

4. **Uso singolo del token**: il primo `UPDATE FirmaToken SET UsatoUtc = SYSUTCDATETIME() WHERE Token = @t AND UsatoUtc IS NULL` consuma il token nella stessa transazione della firma impresa. Se l'impresa apre il link due volte, il primo che committa firma, il secondo riceve "Token già utilizzato".

5. **Creazione token atomica con la firma CSE**: l'INSERT in `FirmaToken` avviene **dentro la stessa transazione** di `VerbaleRepository.FirmaCseAsync`. Razionale: invariante "non esiste un verbale `FirmatoCse` senza il suo token impresa". Se la generazione fallisse post-commit, ci sarebbe una finestra di stati incoerenti; con l'atomicità, o la firma CSE va a buon fine e il link è pronto, o niente. Il metodo `FirmaCseAsync` del repository accetta ora anche `tokenId`, `token`, `scadenzaUtc` come parametri input.

6. **`FirmaCseResult` esteso a `(NumeroAssegnato, Anno, TokenImpresa)`**: il manager ritorna il token al chiamante UI (step 10 del wizard) che lo usa per costruire il dialog "Link impresa" con bottone copia. Il dialog mostra l'URL completo `{BaseUrl}/firma-impresa/{token}` derivato dal contesto HTTP.

7. **Audit della firma impresa**: `VerbaleAudit.UtenteId = verbale.CompilatoDaUtenteId` (cioè l'utente CSE che ha creato il verbale, "by proxy"). `Note = "Firma applicata da Impresa via token {tokenId}"`. Razionale: l'impresa non ha account quindi non ha `UtenteId`; rendere la colonna nullable richiederebbe una migration su FK esistenti, ed è over-engineering per un caso semantico chiaro. La nota traccia esplicitamente che la firma è dell'impresa e correla al token usato.

8. **`VerbaleRepository.FirmaImpresaAsync`**: metodo transazionale parallelo a `FirmaCseAsync` ma più semplice: (1) `SELECT Stato WITH (UPDLOCK, HOLDLOCK)` con verifica `Stato == FirmatoCse` (idempotenza), (2) `INSERT` in `Firma` con `Tipo=ImpresaAppaltatrice` (UQ_Firma_VerbaleId_Tipo blinda doppie firme), (3) `UPDATE Verbale SET Stato=FirmatoImpresa, UpdatedAt = SYSUTCDATETIME()` — **Numero e Anno NON cambiano**, sono stati assegnati alla firma CSE, (4) `INSERT` in `VerbaleAudit` (`EventoTipo=Firma`, Note come al punto 7), (5) `UPDATE FirmaToken SET UsatoUtc = SYSUTCDATETIME()`. Niente parametri Numero/Anno: il metodo è puramente una transizione di stato.

9. **`VerbaleManager.FirmaImpresaAsync(Guid token, byte[] pngBytes, string nomeFirmatario, CancellationToken ct)`**: orchestra. Carica il token tramite `IFirmaTokenRepository.GetByTokenAsync`, valida (`null → InvalidTokenException`, `UsatoUtc != null → TokenAlreadyUsedException`, `ScadenzaUtc <= now → TokenExpiredException`), carica verbale, verifica `Stato == FirmatoCse` (eccezione tipizzata se no), salva PNG via `IFirmaStorageService` (path `firme/{verbaleId}/impresa.png`), chiama `repo.FirmaImpresaAsync`. Stesso pattern di `FirmaCseAsync`: PNG orfano accettato in caso di rollback DB.

10. **Pagina pubblica `/firma-impresa/{Token:guid}`** (`Pages/FirmaImpresa.razor`): `@page` con parametro typed Guid, **senza** `[Authorize]` (l'impresa non è loggata). Layout dedicato `PublicLayout.razor` minimalista (no NavMenu, no logout) — la pagina è "fuori" dall'area amministrativa. Flusso: (a) on init valida token (manager) — se KO mostra una pagina di errore con messaggio specifico (scaduto/usato/inesistente); (b) carica verbale e firma CSE come `VerbaleDetailModel`; (c) mostra riepilogo verbale read-only: numero/anno, cantiere, data, impresa, CSE, riassunto esiti/meteo; (d) renderizza la firma CSE inline come `<img src="data:image/png;base64,...">` (vedi punto 11); (e) embed `SignaturePadDialog` (riusato da B.10, parametrizzato con `TipoFirmatario.ImpresaAppaltatrice` e Nominativo pre-compilato dall'anagrafica impresa); (f) Conferma → `FirmaImpresaAsync` → pagina di conferma "Firma registrata. Grazie."

11. **Firma CSE visibile alla pagina anonima — inline base64 invece di endpoint pubblico**: l'endpoint `/api/firme/{id}/{tipo}` resta `RequireAuthorization()`. Per mostrare l'anteprima della firma CSE nella pagina `/firma-impresa/{token}` (utente non autenticato), il server-side rendering della pagina Blazor legge i bytes del PNG via `IFirmaStorageService` e li serializza nella `<img>` come data URL base64. Razionale: evita di esporre un endpoint pubblico aggiuntivo, riduce la superficie d'attacco, e per un PNG firma di ~20 KB il base64 inline è del tutto accettabile (~27 KB nel DOM). Lo stesso pattern si potrebbe estendere alla pagina di conferma "Firma registrata" se servisse mostrare anche la firma impresa renderizzata.

12. **UI step 10 wizard — dialog "Link impresa" post-firma CSE**: dopo `VerbaleManager.FirmaCseAsync` riuscita, invece di redirect immediato a `/`, apre `LinkImpresaDialog.razor` con: messaggio `"Verbale {N}/{AAAA} firmato. Condividi questo link con l'Impresa per la sua firma:"`, `MudTextField ReadOnly` con l'URL completo (`{baseUrl}/firma-impresa/{token}`), bottone primario "Copia link" (JS interop `navigator.clipboard.writeText`) con feedback toast "Link copiato", bottone secondario "Vai alla home" che chiude e naviga. Base URL ricavata da `IHttpContextAccessor` per garantire scheme+host corretto in produzione (proxy/reverse proxy compatibile).

13. **Detail view `/verbali/{id}`** (`Pages/VerbaleDetail.razor`, `[Authorize]`): pagina dedicata read-only per visualizzare un verbale già firmato. Mostra: header con `{Numero}/{Anno}` e stato (chip colorato), sezione anagrafica completa, sezione esito/meteo, checklist (solo voci selezionate, ordinate per sottosezione), prescrizioni in lista, foto in grid `<img>` collegate a `/api/foto/{id}/thumb`, blocco firme con `<img src="/api/firme/{id}/cse">` e `<img src="/api/firme/{id}/impresa">` (entrambe via endpoint autenticato) + nomi firmatari + date. Bottoni: "Torna alla lista" → `/`, "Stampa" (chiama `window.print()` via JS, CSS print-friendly nasconde nav/bottoni). Link dalla Home: click sulla riga di un verbale firmato apre `/verbali/{id}` (oggi click apre il wizard, che continuerà a valere solo per le bozze).

14. **Sicurezza**:
   - Token GUID v7 a 128 bit: probabilità di brute-force trascurabile.
   - Scadenza configurabile + uso singolo + verifica stato del verbale: tre vincoli sovrapposti.
   - HTTPS obbligatorio in produzione (già impostato dal `UseHsts` di B.10).
   - Nessun rate limit specifico sul path `/firma-impresa/{token}` in B.11 (la difesa primaria è l'imprevedibilità del token). Da rivalutare se in osservazione produzione emergono tentativi di brute-force.
   - Il PNG della firma CSE servito inline base64 alla pagina anonima è limitato al solo possessore del token: senza token, non c'è path che esponga firme.

15. **Bug fixato in-flight — `MaximumReceiveMessageSize` SignalR**: durante il test live B.11 è emerso che il bottone "Conferma firma" sulla pagina `/firma-impresa/{token}` rimaneva appeso senza errori visibili. Causa: il default di SignalR per `HubOptions.MaximumReceiveMessageSize` è **32 KB**, ma il PNG base64 generato da `signature_pad` su display HiDPI mobile (Android, devicePixelRatio 2-3x) supera facilmente 32 KB → il messaggio JS interop con il `getDataUrl` veniva droppato silenziosamente dal server e l'`await` lato C# non tornava mai. Lo stesso problema esisteva latente anche in B.10 (firma CSE) ma non si era manifestato perché era stato testato su tablet con display di dimensioni e DPR diversi. Fix in `Program.cs`: `AddInteractiveServerComponents().AddHubOptions(o => o.MaximumReceiveMessageSize = 1024 * 1024)` (1 MB). Vale per tutte le JSInterop call del progetto: foto, firme, futuri payload grandi.

16. **Bug fixato in-flight — clipboard API in contesto non-sicuro**: il bottone "Copia link" del `LinkImpresaDialog` non funzionava in test LAN: l'app gira su `http://192.168.x.y:5192` (non secure context), `navigator.clipboard` è `undefined`, e l'invoke `JS.InvokeVoidAsync("navigator.clipboard.writeText", ...)` lanciava un'eccezione catturata dal `catch` generico — l'utente vedeva solo lo Snackbar warning e doveva selezionare a mano il testo. Fix: piccolo modulo JS `wwwroot/js/clipboard.js` con la funzione `copy(text)` che tenta `navigator.clipboard.writeText` se in secure context, altrimenti cade nel fallback storico `document.execCommand('copy')` via `<textarea>` temporanea (deprecato ma supportato su tutti i browser anche in http). Pattern riusabile per qualsiasi bottone "copia" futuro. In produzione, dove l'app gira su HTTPS, è sempre il path moderno; il fallback resta come safety net per scenari di test/preview.

**Cosa NON è stato fatto in B.11** (di proposito):
- **Notifica automatica all'impresa** (email/SMS): no SMTP config oggi, il CSE condivide il link manualmente con copia in clipboard. Rimandato a B.12+ se il cliente lo richiede.
- **Rigenerazione token**: se l'impresa perde il link, oggi serve generare un nuovo verbale o intervenire manualmente in DB. Una UI "Rigenera link impresa" sul verbale `FirmatoCse` è uno sviluppo successivo.
- **Pre-check inline scadenza nella pagina firma-impresa**: oggi un utente che apre un link scaduto vede subito il messaggio "Link scaduto". Una pagina che mostra anche "Contatta il CSE per un nuovo link" con dettagli del verbale è polish UX, non vincolo funzionale.
- **PDF stampabile del verbale firmato**: il bottone "Stampa" della detail view usa `window.print()` con CSS print-friendly, sufficiente per scaricare un PDF "stampa su PDF" dal browser. Una generazione server-side con QuestPDF / iText è una feature separata.
...
- **Manager (`VerbaleManager.UpdatePrescrizioniAsync`)**: dedup difensiva — se un Id risulta duplicato nella lista in input, ne viene generato uno nuovo (`Guid.CreateVersion7()`) per le occorrenze successive. Cosi' il batch INSERT non incontra mai duplicati anche se l'UI fosse bacata.
