...
public interface IUtenteManager
{
    ...
    // Aggiorna username/email/ruolo/IsAttivo. NON cambia la password (operazione separata).
    Task AggiornaProfiloAsync(Utente utente, CancellationToken ct = default);
    ...
    Task<IReadOnlyList<Utente>> ListaTuttiAsync(CancellationToken ct = default);
}
